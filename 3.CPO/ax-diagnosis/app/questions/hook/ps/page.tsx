...
const AXIS_COLOR = { header: "bg-green-600", badge: "text-green-700 bg-green-50 border-green-200" };
...
export default function HookPSPage() {
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="max-w-3xl mx-auto px-6 pt-28 pb-20">
          ...
          {NAV.map((n, i) => (
            <a key={n.href} href={n.href}
              className={`shrink-0 text-xs font-bold px-4 py-2 rounded-full border transition-colors ${
                i === 3 ? "bg-green-600 border-green-600 text-white" : `bg-white text-gray-500 border-gray-200 ${n.inactive}`
              }`}>
              {n.label}
            </a>
          ))}
        ...
        <div className="mb-10">
          <span className={`text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border ${AXIS_COLOR.badge}`}>PS · 個人 Soft</span>
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 mt-3">ポータブルスキル・スタンス 設問一覧</h1>
          <p className="text-gray-500 text-sm mt-1">Hook 1問 ／ Checkup 4問 ／ Biopsy 16問</p>
        </div>

        <section className="mb-12">
          <div className="flex items-center gap-3 mb-4">
            <span className="bg-green-600 text-white text-xs font-bold px-3 py-1 rounded-full">Depth 1</span>
            <h2 className="text-lg font-extrabold text-gray-900">Hook <span className="text-gray-400 font-normal text-sm">— 1問</span></h2>
          </div>
          <QuestionCard q={PS_HOOK} headerBg={AXIS_COLOR.header} />
        </section>

        <section className="mb-12">
          <div className="flex items-center gap-3 mb-4">
            <span className="bg-green-600 text-white text-xs font-bold px-3 py-1 rounded-full">Depth 2</span>
          ...
          <div className="space-y-4">
            {PS_CHECKUP.map((q) => <QuestionCard key={q.id} q={q} headerBg={AXIS_COLOR.header} />)}
          </div>
        </section>

        <section className="mb-12">
          <div className="flex items-center gap-3 mb-4">
            <span className="bg-green-600 text-white text-xs font-bold px-3 py-1 rounded-full">Depth 3</span>
        ...
        <div className="flex items-center justify-between">
          <a href="/questions/hook/ph" className="text-blue-600 font-semibold text-sm hover:underline">← 前: PH · テクニカル</a>
          <a href="/questions/hook/oh" className="text-gray-500 font-semibold text-sm hover:text-blue-600 hover:underline">最初へ戻る →</a>
        </div>
      </main>
    </div>
  );
